					...
					*/
                    foreach($post->comment_id as $item)
                    {
                        ...
                        if ($update->loaded == true) {
							...
							$update->comment_follow_status = '1'; // HT: Follow status to send
                            $update->save();
                        }
						/*
						...
						$phone = ORM::factory('form_response')->where('incident_id', $comment->incident_id)->where('form_field_id', 2)->select_list('form_response', 'id');
                ...
                { // Unapprove Action
                    foreach($post->comment_id as $item)
                    {
                        ...
                        if ($update->loaded == true) {
							...
							$update->comment_follow_status = '0'; // HT: Follow status not send
                            $update->save();
                        }
                    }
                ...
                { // Spam Action
                    foreach($post->comment_id as $item)
                    {
                        ...
                        if ($update->loaded == true) {
                            $update->comment_spam = '0';
                            $update->comment_active = '1';
							$update->comment_follow_status = '1'; // HT: Follow status not send
                            $update->save();
                        }
                    }
