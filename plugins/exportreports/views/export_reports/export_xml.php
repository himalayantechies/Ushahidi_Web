...
<channel>
	...
	foreach ($incidents as $incident) {
		echo '<item>';
			...
			echo '<content type="xhtml" xml:lang="en"><![CDATA['
			...
			$custom_fields = customforms::get_custom_form_fields($incident->incident_id, NULL, FALSE, "filter");
			...
			if ($incident->actionable == 1) {
				echo '<actionable>Actionable</actionable>';
			} else if($incident->actionable == 2) {
				echo '<actionable>Urgent</actionable>';
			} else if($incident->actionable == 0) {
				echo '<actionable>Unactionable</actionable>';
			}
			if ($incident->action_urgent) {
				echo '<action_urgent>YES</action_urgent>';
			} else {
				echo '<action_urgent>NO</action_urgent>';
			}
			if ($incident->action_taken) {
				echo '<action_taken>YES</action_taken>';
			} else {
				echo '<action_taken>NO</action_taken>';
			}
			if ($incident->action_closed) {
				echo '<action_closed>YES</action_closed>';
			} else {
				echo '<action_closed>NO</action_closed>';
			}
			if(!empty($incident->action_summary)) echo '<action_summary>'.exportreports_helper::_csv_text($incident->action_summary).'</action_summary>';
			if(!empty($incident->comment_description)) echo '<comment>'.exportreports_helper::_csv_text($incident->comment_description).'</comment>';
			if(!empty($incident->comment_date)) echo '<comment_date>'.exportreports_helper::_csv_text($incident->comment_date).'</comment_date>';
